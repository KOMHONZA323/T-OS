...
set_vbe_mode:
    ...
    ; Attempt 1: Try Selected Resolution
    mov al, [SELECTED_RES]
    cmp al, 0
    je try_720p
    cmp al, 1
    ...
    cmp al, 2
    je try_1440p
    jmp try_1440p ; Default fallback

try_720p:
    mov cx, 1280
    mov dx, 720
    call find_and_set_mode
    jnc mode_success ; Carry clear = success
...
try_1440p:
    ...
    ; If 1440p failed, try 1080p
    mov cx, 1920
    mov dx, 1080
    call find_and_set_mode
    jnc mode_success

    ; If 1080p failed, try 720p
    mov cx, 1280
    mov dx, 720
    call find_and_set_mode
    jnc mode_success
...
mode_success:
    ...
    ; Save Requested Resolution (New Field at +11)
    mov al, [SELECTED_RES]
    mov [es:di+11], al
...
; ------------------------------------------------------------------
; find_and_set_mode
; Input: CX = Width, DX = Height
; Output: Carry Clear (NC) on success, Carry Set (C) on failure
; ------------------------------------------------------------------
find_and_set_mode:
    pusha ; Save all registers

    ; Reset SI to iterate from start of Video Mode List
    mov ax, [VBE_INFO_BLOCK + 14]
    mov fs, [VBE_INFO_BLOCK + 16]
    mov si, ax
